...
Public Class frm_facprv
    ...
    Private Sub frm_facprv_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        inicializa()
    End Sub
    ...
    Private Sub busca_correlativo()
        cfacturaprv.correlativo_siguiente(tipodoc, str_cnn)
        If cfacturaprv.Aerror = False Then
            If cfacturaprv.Aexiste Then
                correlativo = cfacturaprv.Acorrelativo
    ...
    Private Sub graba_datos()
        ...
        cfacturaprv.nuevo(numero, _
                          ...
                          str_cnn)
        If cfacturaprv.Aerror = False Then
            cfacturaprv.buscar_ultima(ordencp, str_cnn)
            cfacturaprv.actualiza_correlativo("FACPRV", str_cnn)
            If cfacturaprv.Aerror = False And cfacturaprv.Aexiste = True Then
                With dtg_movimientos
                    For a = 0 To .RowCount - 1
                        If .Rows(a).Cells(6).Value <> 0.0 Then
                            ...
                            cfacturaprv.nuevo_detalle(cfacturaprv.Aultima, orden_articulo, descripcion, 1, costo, existencia, cantidad, impuesto, totallinea, precio, str_cnn)
                        End If
                    Next
                End With
            End If
    ...
    Private Sub muestra_datos()
        Dim vendedor As Integer = 0
        cfacturaprv.busca(orden_doc, str_cnn)
        If cfacturaprv.Aexiste = True Then
            'vendedor = cfacturaprv.acodigovnd
            'cvendedor.buscar(vendedor, str_cnn)
            txt_orden.Text = orden_doc
            txt_numero.Text = cfacturaprv.Anumerdoc
            txt_codigo.Text = cfacturaprv.acodigo
            txt_nombre.Text = cfacturaprv.anombrecp
            txt_nit.Text = cfacturaprv.Anit
            txt_direccion.Text = cfacturaprv.Adireccion
            txt_obs.Text = cfacturaprv.Aobs
            dtp_documento.Value = cfacturaprv.afechadoc
            dtp_vence.Value = cfacturaprv.Afechaven
            txt_vendedor.Text = cfacturaprv.acodigovnd
            busca_integracion()
        End If
    End Sub
    ...
    Private Sub busca_integracion()
        ...
        Dim txt_sql = "select  a.codigo," _
                    ...
                    & "d.precio_venta," _
                    & "d.total_linea " _
                    & " from tbl_prv_factura_det d inner join tbl_articulo a on a.orden = d.orden_articulo " _
    ...
    Private Sub btn_buscar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_buscar.Click
        Dim consulta As New frm_bus_FACPRV
        consulta.orden = 0
        consulta.ShowDialog()
