...
Public Class frm_traslado
    ...
    Private Sub frm_traslado_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        inicializa()
    End Sub
    ...
    Private Sub botones_agregar()
        Me.Text = "Agregando traslado de almacen"
        modo = 1
        ' botones al agregar 
    ...
    Private Sub habilita_campos()
        ...
        txt_almOrig.Enabled = True
        txt_almdes.Enabled = True
        txt_nomorig.Enabled = False
        ...
        If txt_codigo.Text <> "" Then
            If txt_almOrig.Text <> "" Then
                If txt_almdes.Text <> "" Then
                    If txt_exiorig.Text <> "" Then
                        If txt_cantidad.Text <> "" And Val(txt_cantidad.Text) <= Val(txt_exiorig.Text) Then
                            datos_validos = True
                        Else
                            MessageBox.Show("Cantidad a trasladar no es valida", "Verificando cantidad", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        End If
                    Else
                        MessageBox.Show("Existencia actual origen no es valida", "Verificando existencia almacen origen", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End If
                Else
    ...
    Private Sub graba_datos()
        If modo = 1 Then
            MessageBox.Show("Confirma grabacion de traslado de mercaderia", "Grabando traslado de mercaderia", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
            If vbYes Then
                ctraslado.agrega_traslado(ordenart, ordalmorig, ordalmdest, Val(txt_cantidad.Text), str_cnn)
    ...
    Private Sub txt_cantidad_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_cantidad.LostFocus
        If txt_cantidad.Text <> "" Then
            If Val(txt_cantidad.Text) > 0 Then
                If Val(txt_cantidad.Text) > existenciaorig Then
                    MessageBox.Show("No puede trasladar mas unidades de las disponibles", "Indicando cantidad a trasladar", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    txt_cantidad.Text = existenciaorig
                    txt_cantidad.Focus()
                End If
            End If
        End If
