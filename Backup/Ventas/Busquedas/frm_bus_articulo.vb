...
Public Class frm_bus_articulo
    ...
    Private Sub frm_bus_articulo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        resultado = False
        inicializa()
        crea_qry()
        llena_grid()
    ...
    Private Sub crea_qry()
        If txt_nombre.Text = "" Then
            If txt_codigo.Text = "" Then
                txt_sql = "select codigo, descripcion from tbl_articulo order by codigo"
            Else
                txt_sql = "select codigo, descripcion from tbl_articulo where codigo like '%" & txt_codigo.Text & "%' order by codigo"
            End If
        Else
            If txt_nombre.Text <> "" Then
                txt_sql = "select codigo, descripcion from tbl_articulo where descripcion like '%" & txt_nombre.Text & "%' order by descripcion"
            End If
        End If
    End Sub
    ...
    Private Sub llena_grid()
        ...
        If myReader.HasRows Then
            ...
            While myReader.Read()
                dtg_articulos.Rows.Add(myReader.GetValue(0), myReader.GetValue(1))
            End While
