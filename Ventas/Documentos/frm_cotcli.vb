...
Public Class frm_cotcli
    ...
    Private Sub frm_cotcli_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        inicializa()
    End Sub
    ...
    Private Sub busca_correlativo()
        ccotizacion.correlativo_siguiente(tipodoc, str_cnn)
        If ccotizacion.Aerror = False Then
            If ccotizacion.Aexiste Then
                correlativo = ccotizacion.Acorrelativo
    ...
    Private Sub graba_datos()
        ...
        ccotizacion.nuevo(numero, _
                          ...
                          str_cnn)
        If ccotizacion.Aerror = False Then
            ccotizacion.buscar_ultima(ordencp, str_cnn)
            ccotizacion.actualiza_correlativo("COTCLI", str_cnn)
            If ccotizacion.Aerror = False And ccotizacion.Aexiste = True Then
                With dtg_movimientos
                    For a = 0 To .RowCount - 1
                        If .Rows(a).Cells(6).Value <> 0.0 Then
                            ...
                            ccotizacion.nuevo_detalle(ccotizacion.Aultima, orden_articulo, descripcion, 1, costo, existencia, cantidad, impuesto, totallinea, precio, str_cnn)
                        End If
                    Next
                End With
            End If
    ...
    Private Sub btn_buscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_buscar.Click
        Dim consulta As New frm_bus_cotizacion
        consulta.orden = 0
        consulta.ShowDialog()
    ...
    Private Sub muestra_datos()
        Dim vendedor As Integer = 0
        ccotizacion.busca(orden_doc, str_cnn)
        If ccotizacion.Aexiste = True Then
            'vendedor = ccotizacion.acodigovnd
            'cvendedor.buscar(vendedor, str_cnn)
            txt_orden.Text = orden_doc
            txt_numero.Text = ccotizacion.Anumerdoc
            txt_codigo.Text = ccotizacion.acodigo
            txt_nombre.Text = ccotizacion.anombrecp
            txt_nit.Text = ccotizacion.Anit
            txt_direccion.Text = ccotizacion.Adireccion
            txt_obs.Text = ccotizacion.Aobs
            dtp_documento.Value = ccotizacion.afechadoc
            dtp_vence.Value = ccotizacion.Afechaven
            txt_vendedor.Text = ccotizacion.acodigovnd
            busca_integracion()
        End If
    End Sub
    ...
    Private Sub busca_integracion()
        ...
        Dim txt_sql = "select  a.codigo," _
                    ...
                    & "d.precio_venta," _
                    & "d.total_linea " _
                    & " from tbl_cli_cotiza_det d inner join tbl_articulo a on a.orden = d.orden_articulo " _
