...
Public Class pedido
    ...
    Private TFecha_pagado As String
    Private TNotas_pago As String
    Private Tfacturado As Integer
    ...
    Public Sub nuevo(ByVal tnumero As Integer, _
        ...
        Try
            ...
            Ttxt_sql = "INSERT INTO tbl_cli_pedido " _
                   & " (" _
                   & "orden_cp," _
                   & "nombre_cp," _
                   & "direccion," _
    ...
    Public Sub actualiza_existencia(ByVal torden As String, _
                                    ByVal tcantidad As Decimal, _
                                    ByVal talmacen As Integer, _
              ByVal tstc As String)
        terror = False
        Try
            Tcnn = New SqlConnection(tstc)
            ...
            Ttxt_sql = "update tbl_almacendet set existencia = existencia - " & tcantidad _
                   & "  where orden_almacen = " & talmacen & " and orden_articulo = " & torden
            'MsgBox(Ttxt_sql)
            Tcmm = New SqlCommand(Ttxt_sql, Tcnn)
            Tcmm.CommandText = Ttxt_sql
            Tcmm.ExecuteNonQuery()
        Catch ex As Exception
            terror = True
            txtmsg = "No se actualizo existencia: " & ex.Message
            MsgBox(txtmsg, MsgBoxStyle.Information)
        End Try
        Tcnn.Close()
    End Sub

    Public Sub regresa_existencia(ByVal torden As String, _
                                ByVal tcantidad As Decimal, _
                                ByVal talmacen As Integer, _
          ByVal tstc As String)
        terror = False
    ...
    Public Sub nuevo_detalle(ByVal tultimo As Integer, _
        ...
        Try
            ...
            Ttxt_sql = "INSERT INTO tbl_cli_pedido_det " _
                   & " (" _
                   & "orden_doc," _
                   & "orden_articulo," _
                   & "descripcion," _
    ...
    Public Sub busca(ByVal Torden As String, ByVal Tstc As String)
        ...
        Ttxt_sql = "select z.orden," _
                 ...
                 & "isnull((select codigo from tbl_cliente x where x.orden = z.orden_cp),'0'), " _
                 & "z.numero_documento, " _
                 & "z.estado, " _
                 & "z.fecha_pagado, " _
                 & "z.Notas_pago, " _
                 & "z.facturado " _
                 & " from tbl_cli_pedido z " _
            ...
            With Tdtr
                If .HasRows Then
                    ...
                    While .Read
                        ...
                        Testado = Tdtr.Item(11)
                        TFecha_pagado = Tdtr.Item(12)
                        TNotas_pago = Tdtr.Item(13)
                        Tfacturado = Tdtr.Item(14)
                    End While
                End If
            End With
            Tcnn.Close()
    ...
    Public Sub anula(ByVal torden_pedido As String, ByVal tstc As String)
        terror = False
        Ttxt_sql = "update tbl_cli_pedido set estado = 'N',fecha_anulacion = current_timestamp where orden =  " & torden_pedido
        Try
            Tcnn = New SqlConnection(tstc)
            Tcnn.Open()
        ...
        Catch ex As Exception
            terror = True
            txtmsg = "No se pudo marcar como anulado el pedido : " & ex.Message
            MsgBox(txtmsg, MsgBoxStyle.Information)
        End Try
        Tcnn.Close()
    End Sub

    Public Sub marca_facturado(ByVal torden_pedido As String, ByVal tstc As String)
        terror = False
        Ttxt_sql = "update tbl_cli_pedido set facturado = 1 where orden =  " & torden_pedido
        Try
            Tcnn = New SqlConnection(tstc)
            Tcnn.Open()
        ...
        Catch ex As Exception
            terror = True
            txtmsg = "No se pudo marcar como facturado el pedido : " & ex.Message
            MsgBox(txtmsg, MsgBoxStyle.Information)
        End Try
        Tcnn.Close()
    End Sub
    ...
    Public Sub cierra(ByVal torden As String, ByVal tstc As String)
        terror = False
        Try
            Tcnn = New SqlConnection(tstc)
            Tcnn.Open()

            Ttxt_sql = "update tbl_cli_pedido set estado = 'C' " _
                   & "  where orden = " & torden
            'MsgBox(Ttxt_sql)
            Tcmm = New SqlCommand(Ttxt_sql, Tcnn)
            Tcmm.CommandText = Ttxt_sql
        ...
        Catch ex As Exception
            terror = True
            txtmsg = "No se actualizo estado de pedido a Cerrado " & ex.Message
            MsgBox(txtmsg, MsgBoxStyle.Information)
        End Try
        Tcnn.Close()
    End Sub
    ...
    Public Function AFecha_pagado() As String
        Return TFecha_pagado
    End Function

    Public Function Aorden_ultimo() As Integer
        Return tultima
    End Function

    Public Function Afacturado() As Integer
        Return Tfacturado
    End Function
